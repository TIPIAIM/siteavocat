...
const HeroSection = styled.section`
  ...
  padding: 4rem 1rem;
  position: relative;
  background-color: hsla(213, 73%, 14.5%, 0.98);
  overflow: hidden;
...
const Particles = ({ mousePosition }) => {
  ...
  const [positions] = useState(() => {
    ...
    for (let i = 0; i < 5000; i++) {
      ...
      positions.push((Math.random() - 0.5) * 0.70);
      positions.push((Math.random() - 0.5) * 10);
    }
  ...
  return (
    <points ref={meshRef}>
      ...
      <pointsMaterial color="rgba(0, 119, 182, 1)" size={0.02} />
    </points>
  );
};
...
const WindParticles = () => {
  ...
  return (
    <points ref={meshRef}>
      ...
      <pointsMaterial color="rgba(2, 11, 22, 0.87)" size={0.05} />
    </points>
  );
};
...
export default AccueilComposant;
